...
class WebSocketManager extends EventEmitter {
	...
	async connect() {
		...
		this.shardQueue = new Set(shards.map(id => 
			new WebSocketShard(this, id))
		);
	...
	async spawnShards() {
		...
		if (!shard.eventsAttached) {
			...
			shard.on(ShardEvents.CLOSE, event => {
				...
				if (shard.sessionID) {
					this.debug(`Session ID present, attempting to immediately reconnect...`, shard);
					this.reconnect(true);
				} else {
			...
			shard.on(ShardEvents.DESTROYED, () => {
				this.debug('Shard was destroyed but there was no WebSocket connection! Reconnecting...', shard);
				this.client.emit(Events.SHARD_RECONNECTING, shard.id);
				this.shardQueue.add(shard);
				this.reconnect();
			});
	...
	destroy() {
		...
		this.debug(`Manager was destroyed by:\n${new Error('MANAGER_DESTROYED').stack}`);
		this.destroyed = true;
